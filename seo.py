...
def extract_meta_details(url, keyword_input=None):
    try:
        ...
        if word_count > 0:
            if keyword_input:
                occurrences = len(re.findall(rf"\b{re.escape(keyword_input.lower())}\b", page_text.lower()))
                keyword_density = (occurrences / word_count * 100)
            elif keywords:
                top_kw = keywords[0]
                occurrences = len(re.findall(rf"\b{re.escape(top_kw)}\b", page_text.lower()))
                keyword_density = (occurrences / word_count * 100)

        return {
            "Word_Count": word_count,
            "Keyword_Density": round(keyword_density, 2),
            "Meta_Title_Length": len(title or ""),
            "Meta_Desc_Length": len(meta_desc or ""),
...
def get_pagespeed_score(url):
    API_KEY = os.getenv("PAGESPEED_API_KEY")  # set this env var
    if not API_KEY:
        return None
    try:
